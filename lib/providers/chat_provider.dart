...
class ChatProvider extends ChangeNotifier {
  ...
  Future<String> loadAsset() async {
    String statdata = 'Voici ta base de données : ';
    String rotdata = '';
    rotdata = await rootBundle.loadString('assets/texts/code penal.txt');
    rotdata.replaceAll("\n", " ");
    statdata = statdata + rotdata;
    rotdata = await rootBundle.loadString('assets/texts/code du travail.txt');
    rotdata.replaceAll("\n", " ");
    statdata = statdata + rotdata;
    rotdata = await rootBundle.loadString('assets/texts/code de la route.txt');
    rotdata.replaceAll("\n", " ");
    statdata = statdata + rotdata;
    rotdata = await rootBundle.loadString('assets/texts/code civil.txt');
    rotdata.replaceAll("\n", " ");
    statdata = statdata + rotdata;
    return statdata;
  }
  ...
  Future<void> setModel() async {
    _contextData = await loadAsset();

    _model = _textModel ??
        GenerativeModel(
          model: setCurrentModel(newModel: 'gemini-1.5-flash-8b'),
          apiKey: ApiService.apiKey,
          systemInstruction:
              Content.system(Constants.instruction + _contextData),
        );
    notifyListeners();
  }
...
//?yeha samma
  ...
  Future<void> sentMessage({
    ...
    List<Content> history = [
      Content.system(Constants.instruction),
    ];
